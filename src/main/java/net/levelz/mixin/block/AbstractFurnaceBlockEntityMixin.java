    ...
    private void getRecipesUsedAndDropExperienceMixin(ServerWorld world, Vec3d pos, CallbackInfoReturnable<List<Recipe<?>>> info) {
        if (ConfigInit.CONFIG.furnaceXPMultiplier > 0.0F) {
            for (Object2IntMap.Entry<Identifier> entry : this.recipesUsed.object2IntEntrySet()) {
                world.getRecipeManager().get(entry.getKey()).ifPresent(recipeEntry -> {
                    Recipe<?> recipe = recipeEntry.value();
                    if (!recipe.getResult(world.getRegistryManager()).isIn(TagInit.RESTRICTED_FURNACE_EXPERIENCE_ITEMS)) {
                        int i = MathHelper.floor(entry.getIntValue() * ((AbstractCookingRecipe) recipe).getExperience());
                        float f = MathHelper.fractionalPart(entry.getIntValue() * ((AbstractCookingRecipe) recipe).getExperience());
